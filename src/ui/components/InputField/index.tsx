			...
			<div className="">
				<div onClick={click} className={`custom-input ${!true && "full"} `}>
					<textarea
						...
						onBlur={blur}
						className={`textarea ${filedValue.length > 0 || (String(value).length > 0 && "active")} ${
							touched && error && "error"
						}`}
						{...rest}
					/>

					<label className="absolute body-sm text-txt-subdued-alt" htmlFor={label}>
						{label}
					</label>
				</div>
				...
				{touched && error && (
					<div className="error text-txt-critical middle mt-2">
						<img width={20} height={20} src={"/icons/error.svg"} alt="" />
	...
	return (
		<div className="">
			<div onClick={click} className={`custom_input relative`}>
				{type == "password" && (
					<div role={"button"} onClick={() => togglePasswordMode()} className="icon absolute  right-3 bottom-3">
